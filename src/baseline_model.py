...
def age_reg_baseline_model(train, test):
    '''
...
def age_clf_baseline_model(train, test):
    '''
    Age baseline model. Use sklearn DummyClassifier.
    ...
    '''
    dummy_clf = DummyClassifier(strategy='stratified')
    X_train_age = train.drop(columns = ['age_group','age'])
    y_train_age = train.age_group
    dummy_clf.fit(X_train_age, y_train_age)
    X_test_age = test.drop(columns = ['age_group','age'])
    y_true_age = test.age_group.to_numpy()
    y_pred_age = dummy_clf.predict(X_test_age)
    age_baseline = round(accuracy_score(y_true_age, y_pred_age),4)
    print(f'Accuracy for the baseline model for age is : {age_baseline}')
    return age_baseline
...
if __name__ == '__main__':
    ...
    age_reg_baseline = age_reg_baseline_model(train, test)
    age_clf_baseline = age_clf_baseline_model(train, test)
